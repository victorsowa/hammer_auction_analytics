...
def convert_bs4_navigable_string_to_string(bs4_navigable_string):
    string = str(bs4_navigable_string)
    return string.replace(u"\xa0", u" ").replace(u"\xc0", u" ")


def get_estimate(lot_estimate, lot_name):
...
def get_result(lot_result, lot_name):
...
def scrape_lot_information(lots_url):
    page = get_page(lots_url)
    soup = get_soup(page.content)
    return soup.find("div", class_="c-lot-index-lots--narrow")


def get_structured_information_from_lots(lots):
    structured_lots = []
...
def get_information_from_lots(lot_url):
